...
import { TUTORINGSUBJECTS } from "../../config/constants";
//icons
...
const handleFilter = () =>{
  ...
  getAllTutors(1,searchFilter.keyword,range[1],range[0],_subjects,days)
}
...
const handleClear = () => {
  ...
  const getAllTutors = async (pageNumber, keyword, max, min, sbj, days) => {
    setLoading(true);
    try {
      const response = await Get(USERS.getAllTutors,null, {
        page: pageNumber
          ? pageNumber.toString()
  ...
  const handlePageChange = (e) => {
    ...
    getAllTutors(Number(e.selected) + 1);
  };
            ...
            <Card className="contactCard2" bordered={false}>
              ...
              >
                ...
                <Select
                  ...
                  options={TUTORINGSUBJECTS.map(item => { return({
                    value:item,
